...
def save_game_log_to_file(board,letters,users,turn,moves):
    ...
    for user in users:
        users_string=users_string+str(user[0])+"," + str(user[1])+"," + str(user[2])+ ";"
    ...
    text_file.write("%s\n" % move_string[0:len(move_string)-1])
    text_file.close()
...
def get_game_log_from_file():
    ...
    for user_non_converted in users_non_converted:
        ...
        users.append((int(users_parts[0]),int(users_parts[1]),users_parts[2]))
